...
function hillel_build_year_payload($post_id)
{
    ...
    if ($grand_total > 0) {
        ...
        if ($missing_total > 0) {
            // Recalcular los percent_of_total de TODOS los segmentos usando el total general como 100%
            foreach ($out['incidents'] as &$it) {
                if ($it['total'] !== null) {
                    $pct_val = ($it['total'] / $grand_total) * 100;
                    $it['percent_of_total'] = rtrim(rtrim(number_format($pct_val, 1, '.', ''), '0'), '.') . '%';
                } else {
                    $it['percent_of_total'] = null;
                }
            }
            unset($it);
...
/**
 * Calcula la variación porcentual de un TIPO de incidente respecto al AÑO ANTERIOR.
 *
 * Uso:
 *   hillel_get_prev_year_incident_delta(get_the_ID(), 'hate_speech');
 *
 * Retorna:
 *   [
 *     'key'         => 'hate_speech',
 ...
 *     'percent'     => '26.3',      // sin símbolo
 *     'percent_txt' => '26.3%',     // con símbolo
 *     'current'     => 641,         // total actual de ese tipo
 *     'previous'    => 289          // total previo de ese tipo
 *   ]
 *   ó null si no hay año anterior, no existe la clave, valores vacíos, o cambio 0%.
 *
 * @param int    $current_post_id  Post actual
 * @param string $incident_key     Clave del tipo ('hate_speech','social_media_email', etc.)
 * @return array|null
 */
function hillel_get_prev_year_incident_delta(int $current_post_id, string $incident_key)
{
    ...
    // datos actuales (fila master por año)
    $year_title = get_the_title($current_post_id);
    $table_row  = get_years_data('data_table', 'single_year', $year_title, 'data_interactive');
    if (!is_array($table_row) || empty($table_row)) return null;

    // detectar año anterior
    $prev_post = hillel_get_previous_year_post($current_post_id);
    if (!$prev_post) return null;

    $prev_table_row = get_years_data('data_table', 'single_year', get_the_title($prev_post->ID), 'data_interactive');
    if (!is_array($prev_table_row) || empty($prev_table_row)) return null;

    // validar que el key exista en la ventana 2..7 (incidents)
    $keys           = array_keys($table_row);
    $incidents_keys = array_slice($keys, 2, 6, true); // posiciones 2..7

    if (!in_array($incident_key, $incidents_keys, true)) {
        // Si no está exactamente en la ventana, intentar acceso directo (por si el orden cambia)
        if (!array_key_exists($incident_key, $table_row)) return null;
    }

    // valores actual y previo
    $cur  = isset($table_row[$incident_key]) && $table_row[$incident_key] !== '' ? (int)$table_row[$incident_key] : null;
    $prev = isset($prev_table_row[$incident_key]) && $prev_table_row[$incident_key] !== '' ? (int)$prev_table_row[$incident_key] : null;

    if ($cur === null || $prev === null || $prev <= 0) return null;
    if ($cur === $prev) {
        return [
            'key'         => $incident_key,
            'name'        => $fmt($incident_key),
            'type'        => 'equal',
            'percent'     => '0',
            'percent_txt' => '0%',
            'current'     => $cur,
    ...
    return [
        'key'         => $incident_key,
        'name'        => $fmt($incident_key),
        'type'        => ($cur > $prev) ? 'increase' : 'decrease',
        'percent'     => $percent_str,
        'percent_txt' => $percent_str . '%',
        'current'     => $cur,
        'previous'    => $prev,
