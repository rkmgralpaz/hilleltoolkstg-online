...
const startHeader = (block) => {
    ...
    let hideRatio = 70; // Configurable
    let showRatio = 70; // Configurable
    let isHeaderHidden = false;
    let upAccumulator = 0;
    let downAccumulator = 0;
    let scrollAccumulatorTimeout;
    //
    let footerSocial = document.querySelector('.footer__social');
    let socialButton = !!footerSocial ? '<div class="site-secondary-nav-desktop__social theme theme--neutral theme--mode-dark">' + footerSocial.innerHTML + '</div>' : '';
    block.querySelectorAll('.site-secondary-nav-desktop').forEach(el => {
        el.innerHTML = secondaryNavMobile.innerHTML.split('-mobile').join('-desktop') + socialButton;
    });
    ...
    window.addEventListener('scroll', e => {
        clearTimeout(scrollAccumulatorTimeout);
        scrollTop = Math.min(scrollMaxY, Math.max(0, window.scrollY));
        const scrollDifference = scrollTop - prevScrollTop;

        const pageTimelineMobile = pageTimeline && window.innerWidth < 768;

        if (scrollTop > hideRatio && scrollIsTop) {
            scrollIsTop = 0;
            document.body.classList.add('scrolled');
            //logoLottie.setDirection(1);
            //logoLottie.play();
        } else if (scrollTop <= hideRatio && !scrollIsTop) {
            scrollIsTop = 1;
        ...
        if (scrollTop <= 0) {
            ...
            // Hacia abajo
            upAccumulator = 0; // Reset opuesto
            downAccumulator += Math.abs(scrollDifference);
            //
            if (!isHeaderHidden && downAccumulator >= hideRatio) {
                hideHeader();
                resetAccumulators();
            }
        } else if (scrollDifference < 0) {
            // Hacia arriba
            downAccumulator = 0; // Reset opuesto
            upAccumulator += Math.abs(scrollDifference);
            //
            if (isHeaderHidden && upAccumulator >= showRatio) {
                showHeader();
                resetAccumulators();
            }
        }
        ...
        if (pageTimelineMobile) {
            if (!isHeaderHidden && scrollTop > hideRatio) {
                document.body.classList.add('header-hide');
                isHeaderHidden = true;
            }
            if (prevScrollTop > scrollTop) {
        ...
        //if(window.innerWidth >= 1024 && scrollTop > 400){
        if(scrollTop > 400){
            hideHeader();
            //helper.log('hide', true);
        }else{
            //document.body.classList.add('header-hide');
            //helper.log('show', true);
        }

        

        scrollAccumulatorTimeout = setTimeout(resetAccumulators, 300);
